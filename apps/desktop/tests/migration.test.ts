...
describe('migration', () => {
  ...
  it('已含 P0 表的 DB 升级：迁移加 memories 表 + kind 列 + embedding 列，不丢数据', () => {
    const db = freshDb();
    // 模拟老 P0 DB：无 schema_version 表，无 memories 表，messages 无 kind 列，providers 无 embedding 列，但有数据
    db.exec(
    ...
    expect(row.kind).toBe('message');
    // providers 的 embedding_model / context_window 列已加上
    const pcols = db.prepare('PRAGMA table_info(providers)').all() as { name: string }[];
    expect(pcols.some((c) => c.name === 'embedding_model')).toBe(true);
    expect(pcols.some((c) => c.name === 'context_window')).toBe(true);
    // memories 表已加
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as {
      name: string;
    }[];
